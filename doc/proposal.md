...
|---|---|---|---|---|
| **Workstream – Data & ETL** | Import Spring 2025 course/section CSV; cleaning & de-dup; schema mapping to `term/course/section`; seed scripts for synthetic users/posts/teams; snapshots. | N/A | **Ning Wei (ningwei3)** | **Yanlin Tao (tao17)** |
| **Workstream – Backend APIs (Core)** | FastAPI setup; auth plumbing; common DTOs; pagination & validation; error handling; OpenAPI docs. | `GET /health`, `GET /version` | **Jack Jiang (jackj6)** | **Lixuan Gu (lixuang2)** |
| **Workstream – Matching & Workflow** | Team capacity logic (open/full/locked/closed); request lifecycle (create/accept/reject/withdraw/expire); side-effects (auto-join); basic notifications (optional). | `PATCH /api/teams/:id`, `PATCH /api/requests/:id` | **Lixuan Gu (lixuang2)** | **Jack Jiang (jackj6)** |
| **Workstream – Frontend UI/UX** | React app: Course Space, Post Detail, Team Detail, Inbox, Profile; forms & validation; routing; basic accessibility. | N/A | **Yanlin Tao (tao17)** | **Lixuan Gu (lixuang2)** |
| **Workstream – DevOps & Release** | Docker/Compose; env configs; GitHub Actions (lint/test/build); deploy (Render/AWS); runtime logging. | N/A | **Jack Jiang (jackj6)** | **Ning Wei (ningwei3)** |
| **Workstream – QA / Docs / PM** | Test plan; API contract tests; seed-based demo scenarios; proposal & README; milestone tracking & demos. | N/A | **Ning Wei (ningwei3)** | **Yanlin Tao (tao17)** |
| **Module – Auth & Users** | Mock NetID → JWT; profile read/update; skills & availability upsert. | `POST /api/auth/login`, `GET /api/me`, `PUT /api/me`, `PUT /api/me/skills`, `PUT /api/me/availability`, `GET /api/users/:id` | **Jack Jiang (jackj6)** | **Ning Wei (ningwei3)** |
| **Module – Courses & Sections** | Terms/courses/sections listing; course search; ETL import CLI; snapshots. | `GET /api/terms`, `GET /api/courses`, `GET /api/sections` | **Ning Wei (ningwei3)** | **Jack Jiang (jackj6)** |
| **Module – Posts & Search** | Post CRUD; filters (term/course/section/status/skills/q); sorting & pagination. | `POST /api/posts`, `GET /api/posts`, `GET /api/posts/:id`, `PUT /api/posts/:id`, `PATCH /api/posts/:id/status` | **Lixuan Gu (lixuang2)** | **Yanlin Tao (tao17)** |
| **Module – Teams & Members** | Team CRUD; capacity & status; add/remove members; open slots derivation. | `POST /api/teams`, `GET /api/teams`, `GET /api/teams/:id`, `PATCH /api/teams/:id`, `POST /api/teams/:id/members`, `DELETE /api/teams/:id/members/:user_id` | **Yanlin Tao (tao17)** | **Ning Wei (ningwei3)** |
| **Module – Match Requests** | Create user→user / user→team (XOR); inbox/sent; accept/reject/withdraw; expiry. | `POST /api/requests`, `GET /api/requests/inbox`, `GET /api/requests/sent`, `PATCH /api/requests/:id` | **Lixuan Gu (lixuang2)** | **Jack Jiang (jackj6)** |
