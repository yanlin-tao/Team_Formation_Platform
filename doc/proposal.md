...
| **Workstream – Matching & Workflow** | Team capacity logic (open/full/locked/closed); request lifecycle (create/accept/reject/withdraw/expire); side-effects (auto-join); basic notifications (optional). | `PATCH /api/teams/:id`, `PATCH /api/requests/:id` | <Name 3> | <Name 4> |
| **Workstream – Frontend UI/UX** | React app: Course Space, Post Detail, Team Detail, Inbox, Profile; forms & validation; routing; basic accessibility. | N/A | <Name 4> | <Name 3> |
| **Workstream – DevOps & Release** | Docker/Compose; env configs; GitHub Actions (lint/test/build); deploy (Render/AWS); runtime logging. | N/A | <Name 2> | <Name 4> |
| **Workstream – QA / Docs / PM** | Test plan; API contract tests; seed-based demo scenarios; proposal & README; milestone tracking & demos. | N/A | <Name 1> | <Name 3> |
| **Module – Auth & Users** | Mock NetID → JWT; profile read/update; skills & availability upsert. | `POST /api/auth/login`, `GET /api/me`, `PUT /api/me`, `PUT /api/me/skills`, `PUT /api/me/availability`, `GET /api/users/:id` | <Name 2> | <Name 3> |
| **Module – Courses & Sections** | Terms/courses/sections listing; course search; ETL import CLI; snapshots. | `GET /api/terms`, `GET /api/courses`, `GET /api/sections` | <Name 1> | <Name 2> |
| **Module – Posts & Search** | Post CRUD; filters (term/course/section/status/skills/q); sorting & pagination. | `POST /api/posts`, `GET /api/posts`, `GET /api/posts/:id`, `PUT /api/posts/:id`, `PATCH /api/posts/:id/status` | <Name 3> | <Name 4> |
| **Module – Teams & Members** | Team CRUD; capacity & status; add/remove members; open slots derivation. | `POST /api/teams`, `GET /api/teams`, `GET /api/teams/:id`, `PATCH /api/teams/:id`, `POST /api/teams/:id/members`, `DELETE /api/teams/:id/members/:user_id` | <Name 4> | <Name 2> |
| **Module – Match Requests** | Create user→user / user→team (XOR); inbox/sent; accept/reject/withdraw; expiry. | `POST /api/requests`, `GET /api/requests/inbox`, `GET /api/requests/sent`, `PATCH /api/requests/:id` | <Name 3> | <Name 1> |
